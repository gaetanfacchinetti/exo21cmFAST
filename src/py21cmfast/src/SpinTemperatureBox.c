...
// llh
//  Defining tables of energy injected by DM
//double *dxheat_DM_dt_box, *dxion_DM_dt_box, *dxlya_DM_dt_box;
...
int ComputeTsBox(float redshift, float prev_redshift, struct UserParams *user_params, struct CosmoParams *cosmo_params,
                 ...
                 struct InitialConditions *ini_boxes, struct ExoticEnergyInjected *exotic_energy_injected, struct TsBox *this_spin_temp)
{
    ...
    { // This Try{} wraps the whole function.
        ...
        //fprintf(output_file, "# Redshift | J_alpha_X [cm^{2} s^{-1} Hz^{-1} sr^{-1} | J_alpha_star [cm^{2} s^{-1} Hz^{-1} sr^{-1}] | J_alpha_DM [cm^{2} s^{-1} Hz^{-1} sr^{-1}] | Lambda_ion [s^{-1}] | eps_heat (no DM) [erg s^{-1}] | eps_heat_DM [erg s^{-1}]");
        //fclose(output_file);
        ...
        if (!TsInterpArraysInitialised)
        {
            ...
            if (flag_options->USE_MASS_DEPENDENT_ZETA)
            {
                ...
                // llh
                //  Initialising the bowes for the DM energy injection
                //dxheat_DM_dt_box = (double *)calloc(HII_TOT_NUM_PIXELS, sizeof(double));
                //dxion_DM_dt_box = (double *)calloc(HII_TOT_NUM_PIXELS, sizeof(double));
                //dxlya_DM_dt_box = (double *)calloc(HII_TOT_NUM_PIXELS, sizeof(double));
        ...
        if (flag_options->USE_MASS_DEPENDENT_ZETA)
        {
            if (flag_options->USE_MINI_HALOS)
            {
                ...
                Mlim_Fesc_MINI = Mass_limit_bisection(global_params.M_MIN_INTEGRAL, global_params.M_MAX_INTEGRAL, astro_params->ALPHA_ESC, astro_params->F_ESC7_MINI * pow(1e3, astro_params->ALPHA_ESC));
            }
            else
            {
        ...
        else // If flag_options->USE_MASS_DEPENDENT_ZETA == False
        {
            if (flag_options->M_MIN_in_Mass)
                M_MIN = (astro_params->M_TURN) / 50.;
            else // set the minimum source mass
                mu_for_Ts = (astro_params->X_RAY_Tvir_MIN < 9.99999e3) ? 1.22 : 0.6; // neutral IGM : ionized IGM
        }
                ...
                for (i = 0; i < user_params->HII_DIM; i++)
                    for (j = 0; j < user_params->HII_DIM; j++)
                        for (k = 0; k < user_params->HII_DIM; k++)
                        {
                            ...
                            this_spin_temp->Ts_box[HII_R_INDEX(i, j, k)] = get_Ts(redshift, perturbed_field->density[HII_R_INDEX(i, j, k)] * inverse_growth_factor_z * growth_factor_zp, TK, xe, 0, &curr_xalpha);
                        }
            }
        ...
        else // If redshift  <= global_params.Z_HEAT_MAX -> This goes almost to the end of the function
        {
            ...
            if (this_spin_temp->first_box || (fabs(initialised_redshift - perturbed_field_redshift) > 0.0001))
            {
                ...
                for (R_ct = 0; R_ct < global_params.NUM_FILTER_STEPS_FOR_Ts; R_ct++)
                {
                    ...
                    memcpy(box, unfiltered_box, sizeof(fftwf_complex) * HII_KSPACE_NUM_PIXELS); // Copy over unfiltered box

                    if (R_ct > 0)// don't filter on cell size
                        filter_box(box, 1, global_params.HEAT_FILTER, R);
                    
                    dft_c2r_cube(user_params->USE_FFTW_WISDOM, user_params->HII_DIM, user_params->N_THREADS, box);// fft back to real space
                        ...
                        for (i = 0; i < user_params->HII_DIM; i++)
                        {
                            for (j = 0; j < user_params->HII_DIM; j++)
                            {
                                for (k = 0; k < user_params->HII_DIM; k++)
                                {
                                    ...
                                    curr_delNL0 *= inverse_growth_factor_z; // and linearly extrapolate to z=0
                    ...
                    if (user_params->USE_INTERPOLATION_TABLES)
                    {
                        ...
                        min_density = (min_density < 0.0) ? (min_density * 1.01) : (min_density * 0.99);
                        max_density = (max_density < 0.0) ? (max_density * 0.99) : (max_density * 1.01);
            ...
            dzp = (this_spin_temp->first_box) ? (zp - prev_zp) : (redshift - prev_redshift);
            determine_zpp_min = perturbed_field_redshift * 0.999;

            for (R_ct = 0; R_ct < global_params.NUM_FILTER_STEPS_FOR_Ts; R_ct++)
            {
                prev_zpp = (R_ct == 0) ? zp : zpp_edge[R_ct - 1];
                prev_R   = (R_ct == 0) ? 0  : R_values[R_ct - 1];
            ...
            if (this_spin_temp->first_box || (fabs(initialised_redshift - perturbed_field_redshift) > 0.0001))
            {
                ...
                if (user_params->USE_INTERPOLATION_TABLES)
                {
                    if (flag_options->USE_MASS_DEPENDENT_ZETA)
                    {
                        ...
                        {
                            initialise_Nion_Ts_spline_MINI(zpp_interp_points_SFR, determine_zpp_min, determine_zpp_max, astro_params->ALPHA_STAR, astro_params->ALPHA_STAR_MINI, astro_params->ALPHA_ESC, astro_params->F_STAR10, astro_params->F_ESC10, astro_params->F_STAR7_MINI, astro_params->F_ESC7_MINI);
                            initialise_SFRD_spline_MINI(zpp_interp_points_SFR, determine_zpp_min, determine_zpp_max, astro_params->ALPHA_STAR, astro_params->ALPHA_STAR_MINI, astro_params->F_STAR10, astro_params->F_STAR7_MINI);
                        }
                    ...
                    else
                    {
                        ...
                        #pragma omp parallel shared(determine_zpp_min, determine_zpp_max, Sigma_Tmin_grid, ST_over_PS_arg_grid, mu_for_Ts, M_MIN, M_MIN_WDM) private(i, zpp_grid) num_threads(user_params->N_THREADS)
                        {
                            #pragma omp for
                            for (i = 0; i < zpp_interp_points_SFR; i++)
                            {
                        ...
                        #pragma omp parallel shared(fcoll_R_grid, dfcoll_dz_grid, Sigma_Tmin_grid, determine_zpp_min, determine_zpp_max, grid_dens, sigma_atR) private(ii, i, j, zpp_grid, grid_sigmaTmin, grid_dens_val) num_threads(user_params->N_THREADS)
                        {
                            #pragma omp for
                            for (ii = 0; ii < global_params.NUM_FILTER_STEPS_FOR_Ts; ii++)
                            {
            ...
            if (flag_options->USE_MASS_DEPENDENT_ZETA)
            {
                ...
                for (R_ct = 0; R_ct < global_params.NUM_FILTER_STEPS_FOR_Ts; R_ct++)
                {
                    prev_zpp = (R_ct == 0) ? redshift : zpp_edge[R_ct - 1];
                    prev_R = (R_ct == 0) ? 0 : R_values[R_ct - 1];
                ...
                {
                    if (!flag_options->USE_MINI_HALOS)
                        initialise_SFRD_Conditional_table(global_params.NUM_FILTER_STEPS_FOR_Ts, min_densities, max_densities, zpp_growth, R_values, astro_params->M_TURN, astro_params->ALPHA_STAR, astro_params->F_STAR10, user_params->FAST_FCOLL_TABLES);
                    else
                        initialise_SFRD_Conditional_table_MINI(global_params.NUM_FILTER_STEPS_FOR_Ts, min_densities, max_densities, zpp_growth, R_values, Mcrit_atom_interp_table, astro_params->ALPHA_STAR, astro_params->ALPHA_STAR_MINI, astro_params->F_STAR10, astro_params->F_STAR7_MINI, user_params->FAST_FCOLL_TABLES);
                }
            }
            ...
            if (flag_options->USE_MASS_DEPENDENT_ZETA)
            {
                ...
                {
                    if (flag_options->USE_MINI_HALOS)
                        Splined_Fcollzp_mean = Nion_General(zp, global_params.M_MIN_INTEGRAL, atomic_cooling_threshold(zp), astro_params->ALPHA_STAR, astro_params->ALPHA_ESC, astro_params->F_STAR10, astro_params->F_ESC10, Mlim_Fstar, Mlim_Fesc);
                    else
                        Splined_Fcollzp_mean = Nion_General(zp, M_MIN, astro_params->M_TURN, astro_params->ALPHA_STAR, astro_params->ALPHA_ESC, astro_params->F_STAR10, astro_params->F_ESC10, Mlim_Fstar, Mlim_Fesc);
                }
                ...
                if (flag_options->USE_MINI_HALOS)
                {
                            ...
                            for (j = 0; j < user_params->HII_DIM; j++)
                            {
                                for (k = 0; k < user_params->HII_DIM; k++)
                                {
                                    ...
                                    if (flag_options->FIX_VCB_AVG)
                                        curr_vcb = global_params.VAVG; // with this flag we ignore reading vcb box
                                    else // If (user_params->USE_RELATIVE_VELOCITIES == False ) set vcb to a constant, either zero or vavg.                               
                                        curr_vcb = (user_params->USE_RELATIVE_VELOCITIES) ? ini_boxes->lowres_vcb[HII_R_INDEX(i, j, k)] :  0.0;
                                    
                                    *((float *)log10_Mcrit_LW_unfiltered + HII_R_FFT_INDEX(i, j, k)) = log10(lyman_werner_threshold(zp, previous_spin_temp->J_21_LW_box[HII_R_INDEX(i, j, k)], curr_vcb, astro_params));
                    ...
                    if (user_params->USE_INTERPOLATION_TABLES)
                    {
                        ...
                        Splined_Fcollzp_mean_MINI_left = Nion_z_val_MINI[redshift_int_Nion_z + zpp_interp_points_SFR * log10_Mcrit_LW_ave_int_Nion_z] + (zp - redshift_table_Nion_z) / (zpp_bin_width) * (Nion_z_val_MINI[redshift_int_Nion_z + 1 + zpp_interp_points_SFR * log10_Mcrit_LW_ave_int_Nion_z] - Nion_z_val_MINI[redshift_int_Nion_z + zpp_interp_points_SFR * log10_Mcrit_LW_ave_int_Nion_z]);
                        Splined_Fcollzp_mean_MINI_right = Nion_z_val_MINI[redshift_int_Nion_z + zpp_interp_points_SFR * (log10_Mcrit_LW_ave_int_Nion_z + 1)] + (zp - redshift_table_Nion_z) / (zpp_bin_width) * (Nion_z_val_MINI[redshift_int_Nion_z + 1 + zpp_interp_points_SFR * (log10_Mcrit_LW_ave_int_Nion_z + 1)] - Nion_z_val_MINI[redshift_int_Nion_z + zpp_interp_points_SFR * (log10_Mcrit_LW_ave_int_Nion_z + 1)]);
                        ...
                        Splined_Fcollzp_mean_MINI = Nion_General_MINI(zp, global_params.M_MIN_INTEGRAL, pow(10., log10_Mcrit_LW_ave), atomic_cooling_threshold(zp),  astro_params->ALPHA_STAR_MINI, astro_params->ALPHA_ESC, astro_params->F_STAR7_MINI, astro_params->F_ESC7_MINI, Mlim_Fstar_MINI, Mlim_Fesc_MINI);
                }
                else
                    Splined_Fcollzp_mean_MINI = 0;
            
                NO_LIGHT = ((Splined_Fcollzp_mean < 1e-15) && (Splined_Fcollzp_mean_MINI < 1e-15)) ? 1 : 0;
            ...
            {   
                if (flag_options->M_MIN_in_Mass)
                {
                    NO_LIGHT = (FgtrM(zp, fmaxf(M_MIN, M_MIN_WDM)) < 1e-15) ? 1 : 0;
                    M_MIN_at_zp = M_MIN;
                }
                else
                {
                    NO_LIGHT = (FgtrM(zp, fmaxf((float)TtoM(zp, astro_params->X_RAY_Tvir_MIN, mu_for_Ts), M_MIN_WDM)) < 1e-15) ? 1 : 0;
            ...
            for (R_ct = 0; R_ct < global_params.NUM_FILTER_STEPS_FOR_Ts; R_ct++)
            {
                ...
                dzpp_for_evolve = (R_ct == 0) ? (zp - zpp_edge[0]) : (zpp_edge[R_ct - 1] - zpp_edge[R_ct]);
                zpp_growth[R_ct] = dicke(zpp);
                ...
                if (flag_options->USE_MASS_DEPENDENT_ZETA)
                {
                    ...
                    if (flag_options->USE_MINI_HALOS)
                    {
                        ...
                        if (R_ct > 0)
                            filter_box(log10_Mcrit_LW_filtered, 1, global_params.HEAT_FILTER, R_values[R_ct]); // don't filter on cell size
                        ...
                        if (user_params->USE_INTERPOLATION_TABLES)
                        {
                            ...
                            Splined_SFRD_zpp_MINI_left = SFRD_val_MINI[redshift_int_SFRD + zpp_interp_points_SFR * log10_Mcrit_LW_ave_int_SFRD] + (zpp - redshift_table_SFRD) / (zpp_bin_width) * (SFRD_val_MINI[redshift_int_SFRD + 1 + zpp_interp_points_SFR * log10_Mcrit_LW_ave_int_SFRD] - SFRD_val_MINI[redshift_int_SFRD + zpp_interp_points_SFR * log10_Mcrit_LW_ave_int_SFRD]);
                            Splined_SFRD_zpp_MINI_right = SFRD_val_MINI[redshift_int_SFRD + zpp_interp_points_SFR * (log10_Mcrit_LW_ave_int_SFRD + 1)] + (zpp - redshift_table_SFRD) / (zpp_bin_width) * (SFRD_val_MINI[redshift_int_SFRD + 1 + zpp_interp_points_SFR * (log10_Mcrit_LW_ave_int_SFRD + 1)] - SFRD_val_MINI[redshift_int_SFRD + zpp_interp_points_SFR * (log10_Mcrit_LW_ave_int_SFRD + 1)]);
                ...
                else
                {
                    ...
                    {
                        sigma_Tmin[R_ct] = (flag_options->M_MIN_in_Mass) ? sigma_z0(fmaxf(M_MIN, M_MIN_WDM)) : sigma_z0(fmaxf((float)TtoM(zpp, astro_params->X_RAY_Tvir_MIN, mu_for_Ts), M_MIN_WDM));
                        ST_over_PS[R_ct] = dzpp_for_evolve * pow(1 + zpp, -(astro_params->X_RAY_SPEC_INDEX));
                    }
                }
                ...
                if (user_params->USE_INTERPOLATION_TABLES)
                {
                    lower_int_limit = (flag_options->USE_MINI_HALOS) ? fmax(nu_tau_one_MINI(zp, zpp, x_e_ave, filling_factor_of_HI_zp, log10_Mcrit_LW_ave, LOG10_MTURN_INT), (astro_params->NU_X_THRESH) * NU_over_EV) : fmax(nu_tau_one(zp, zpp, x_e_ave, filling_factor_of_HI_zp), (astro_params->NU_X_THRESH) * NU_over_EV);
            ...
            if (!user_params->USE_INTERPOLATION_TABLES)
            {
                #pragma omp parallel shared(ST_over_PS, zpp_for_evolve_list, log10_Mcrit_LW_ave_list, Mcrit_atom_interp_table, M_MIN, Mlim_Fstar, Mlim_Fstar_MINI, x_e_ave, filling_factor_of_HI_zp, x_int_XHII, freq_int_heat_tbl, freq_int_ion_tbl, freq_int_lya_tbl, LOG10_MTURN_INT) private(R_ct, x_e_ct, lower_int_limit) num_threads(user_params->N_THREADS)
                {
                    #pragma omp for
                    for (R_ct = 0; R_ct < global_params.NUM_FILTER_STEPS_FOR_Ts; R_ct++)
                    {
                        if (flag_options->USE_MASS_DEPENDENT_ZETA)
                        {
                            if (flag_options->USE_MINI_HALOS)
                            {
                                ST_over_PS[R_ct] *= Nion_General(zpp_for_evolve_list[R_ct], global_params.M_MIN_INTEGRAL, Mcrit_atom_interp_table[R_ct], astro_params->ALPHA_STAR, 0., astro_params->F_STAR10, 1., Mlim_Fstar, 0.);
                                ST_over_PS_MINI[R_ct] *= Nion_General_MINI(zpp_for_evolve_list[R_ct], global_params.M_MIN_INTEGRAL, pow(10., log10_Mcrit_LW_ave_list[R_ct]), Mcrit_atom_interp_table[R_ct], astro_params->ALPHA_STAR_MINI, 0., astro_params->F_STAR7_MINI, 1., Mlim_Fstar_MINI, 0.);
                            }
                            else
                            ...
                            ST_over_PS[R_ct] *= (flag_options->M_MIN_in_Mass) ? FgtrM_General(zpp_for_evolve_list[R_ct], fmaxf(M_MIN, M_MIN_WDM)) : FgtrM_General(zpp_for_evolve_list[R_ct], fmaxf((float)TtoM(zpp_for_evolve_list[R_ct], astro_params->X_RAY_Tvir_MIN, mu_for_Ts), M_MIN_WDM));

                        lower_int_limit = (flag_options->USE_MINI_HALOS) ? fmax(nu_tau_one_MINI(zp, zpp_for_evolve_list[R_ct], x_e_ave, filling_factor_of_HI_zp, log10_Mcrit_LW_ave_list[R_ct], LOG10_MTURN_INT), (astro_params->NU_X_THRESH) * NU_over_EV) : fmax(nu_tau_one(zp, zpp_for_evolve_list[R_ct], x_e_ave, filling_factor_of_HI_zp), (astro_params->NU_X_THRESH) * NU_over_EV); 
            ...
            if (!flag_options->USE_MASS_DEPENDENT_ZETA)
            {
                ...
                else
                {
                    for (R_ct = 0; R_ct < global_params.NUM_FILTER_STEPS_FOR_Ts; R_ct++)
                    {
                        ...
                        #pragma omp parallel shared(dens_grid_int_vals, R_ct, fcoll_interp1, density_gridpoints, delNL0_rev, fcoll_interp2, table_int_boundexceeded_threaded, zpp_for_evolve_list, sigma_Tmin, sigma_atR) private(box_ct) num_threads(user_params->N_THREADS)
                        {
                            ...
                            for (box_ct = 0; box_ct < HII_TOT_NUM_PIXELS; box_ct++)
                            {
                                if (user_params->USE_INTERPOLATION_TABLES)
                                {
                                    ...
                                    fcoll_R_for_reduction += (fcoll_interp1[dens_grid_int_vals[box_ct][R_ct]][R_ct] * (density_gridpoints[dens_grid_int_vals[box_ct][R_ct] + 1][R_ct] - delNL0_rev[box_ct][R_ct]) + fcoll_interp2[dens_grid_int_vals[box_ct][R_ct]][R_ct] * (delNL0_rev[box_ct][R_ct] - density_gridpoints[dens_grid_int_vals[box_ct][R_ct]][R_ct]));
                                }
                                else
            ...
            const_zp_prefactor_MINI = (flag_options->USE_MINI_HALOS) ? ((astro_params->L_X_MINI) * Luminosity_converstion_factor) / ((astro_params->NU_X_THRESH) * NU_over_EV) * C * astro_params->F_STAR7_MINI * cosmo_params->OMb * RHOcrit * pow(CMperMPC, -3) * pow(1 + zp, astro_params->X_RAY_SPEC_INDEX + 3) : 0.;
      
            J_alpha_ave = xalpha_ave = Xheat_ave = Xion_ave = 0.;
            J_alpha_ave_MINI = J_LW_ave = J_LW_ave_MINI = Xheat_ave_MINI = 0.;
            J_alpha_DM_ave = eps_DM_ave = Lambda_ion_DM_ave = 0;    // llh
            ...
            {
                // llh
                #pragma omp parallel shared(del_fcoll_Rct, dxheat_dt_box, dxion_source_dt_box, dxlya_dt_box, dstarlya_dt_box, previous_spin_temp, x_int_XHII, m_xHII_low_box, inverse_val_box, inverse_diff, dstarlyLW_dt_box, dstarlyLW_dt_box_MINI, dxheat_dt_box_MINI, dxion_source_dt_box_MINI, dxlya_dt_box_MINI, dstarlya_dt_box_MINI) private(box_ct, xHII_call) num_threads(user_params->N_THREADS)
                {
                    #pragma omp for
                    for (box_ct = 0; box_ct < HII_TOT_NUM_PIXELS; box_ct++)
                    {
                        ...
                        //dxheat_DM_dt_box[box_ct] = 0.;     // llh
                        //dxion_DM_dt_box[box_ct] = 0.;      // llh
                        //dxlya_DM_dt_box[box_ct] = 0.;      // llh
                ...
                // Loop on the different possible filters
                for (R_ct = global_params.NUM_FILTER_STEPS_FOR_Ts; R_ct--;)
                {
                    if (!user_params->USE_INTERPOLATION_TABLES)
                    {
                    ...
                    {
                        fcoll_interp_min = (min_densities[R_ct] * zpp_growth[R_ct] <= -1.) ? log10(global_params.MIN_DENSITY_LOW_LIMIT) : log10(1. + min_densities[R_ct] * zpp_growth[R_ct]); 
                        fcoll_interp_bin_width = (max_densities[R_ct] * zpp_growth[R_ct] > global_params.CRIT_DENS_TRANSITION) ? (1. / ((float)NSFR_low - 1.) * (log10(1. + global_params.CRIT_DENS_TRANSITION) - fcoll_interp_min)) : (1. / ((float)NSFR_low - 1.) * (log10(1. + max_densities[R_ct] * zpp_growth[R_ct]) - fcoll_interp_min));
                    ...
                    if (user_params->MINIMIZE_MEMORY)
                    {
                        ...
                        if (R_ct > 0)
                            filter_box(box, 1, global_params.HEAT_FILTER, R_values[R_ct]);  // don't filter on cell size
                            ...
                            for (i = 0; i < user_params->HII_DIM; i++)
                                for (j = 0; j < user_params->HII_DIM; j++)
                                    for (k = 0; k < user_params->HII_DIM; k++)
                                    {
                                        curr_delNL0 = *((float *)box + HII_R_FFT_INDEX(i, j, k));
                                        if (curr_delNL0 <= -1)
                                            curr_delNL0 = -1 + FRACT_FLOAT_ERR; // correct for aliasing in the filtering step
                    ...
                    #pragma omp parallel shared(delNL0, zpp_growth, SFRD_z_high_table, fcoll_interp_high_min, fcoll_interp_high_bin_width_inv, log10_SFRD_z_low_table, fcoll_int_boundexceeded_threaded, log10_Mcrit_LW, SFRD_z_high_table_MINI, log10_SFRD_z_low_table_MINI, del_fcoll_Rct, del_fcoll_Rct_MINI, Mmax, sigmaMmax, Mcrit_atom_interp_table, Mlim_Fstar, Mlim_Fstar_MINI) private(box_ct, curr_dens, fcoll, dens_val, fcoll_int, log10_Mcrit_LW_val, log10_Mcrit_LW_int, log10_Mcrit_LW_diff, fcoll_MINI_left, fcoll_MINI_right, fcoll_MINI) num_threads(user_params->N_THREADS)
                    {
                        #pragma omp for reduction(+ : ave_fcoll, ave_fcoll_MINI)
                        for (box_ct = 0; box_ct < HII_TOT_NUM_PIXELS; box_ct++)
                        {
                            curr_dens = (user_params->MINIMIZE_MEMORY) ? (delNL0[0][box_ct] * zpp_growth[R_ct]) : (delNL0[R_ct][box_ct] * zpp_growth[R_ct]);
                            ...
                            if (!NO_LIGHT)
                            {
                                    ...
                                    if (curr_dens < global_params.CRIT_DENS_TRANSITION)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (fcoll_int < 0 || (fcoll_int + 1) > (NSFR_low - 1))
                                            {
                                                if (fcoll_int == (NSFR_low - 1))
                                                {
                                                    if (fabs(curr_dens - global_params.CRIT_DENS_TRANSITION) < 1e-4)
                                                    {
                                                        ...
                                                        if (flag_options->USE_MINI_HALOS)
                                                        {
                                                            fcoll_MINI_left = SFRD_z_high_table_MINI[R_ct][fcoll_int + NSFR_high * log10_Mcrit_LW_int] * (1. + (float)fcoll_int - dens_val) +  SFRD_z_high_table_MINI[R_ct][fcoll_int + 1 + NSFR_high * log10_Mcrit_LW_int] * (dens_val - (float)fcoll_int);
                                                            fcoll_MINI_right = SFRD_z_high_table_MINI[R_ct][fcoll_int + NSFR_high * (log10_Mcrit_LW_int + 1)] * (1. + (float)fcoll_int - dens_val) + SFRD_z_high_table_MINI[R_ct][fcoll_int + 1 + NSFR_high * (log10_Mcrit_LW_int + 1)] * (dens_val - (float)fcoll_int);
                                            ...
                                            else
                                            {
                                                ...
                                                if (flag_options->USE_MINI_HALOS)
                                                {
                                                    fcoll_MINI_left = log10_SFRD_z_low_table_MINI[R_ct][fcoll_int + NSFR_low * log10_Mcrit_LW_int] * (1 + (float)fcoll_int - dens_val) + log10_SFRD_z_low_table_MINI[R_ct][fcoll_int + 1 + NSFR_low * log10_Mcrit_LW_int] * (dens_val - (float)fcoll_int);
                                                    fcoll_MINI_right = log10_SFRD_z_low_table_MINI[R_ct][fcoll_int + NSFR_low * (log10_Mcrit_LW_int + 1)] * (1 + (float)fcoll_int - dens_val) + log10_SFRD_z_low_table_MINI[R_ct][fcoll_int + 1 + NSFR_low * (log10_Mcrit_LW_int + 1)] * (dens_val - (float)fcoll_int);
                                                    fcoll_MINI = fcoll_MINI_left * (1. - log10_Mcrit_LW_diff) + fcoll_MINI_right * log10_Mcrit_LW_diff;
                                                    fcoll_MINI = expf(fcoll_MINI);
                                                }
                                            }
                                ...
                                else
                                {
                                    if (flag_options->USE_MINI_HALOS)
                                    {
                                        fcoll = Nion_ConditionalM(zpp_growth[R_ct], log(global_params.M_MIN_INTEGRAL), log(Mmax), sigmaMmax, Deltac, curr_dens, Mcrit_atom_interp_table[R_ct], astro_params->ALPHA_STAR, 0., astro_params->F_STAR10, 1., Mlim_Fstar, 0., user_params->FAST_FCOLL_TABLES);
                                        fcoll_MINI = Nion_ConditionalM_MINI(zpp_growth[R_ct], log(global_params.M_MIN_INTEGRAL), log(Mmax), sigmaMmax, Deltac, curr_dens, pow(10, log10_Mcrit_LW[R_ct][box_ct]), Mcrit_atom_interp_table[R_ct], astro_params->ALPHA_STAR_MINI, 0., astro_params->F_STAR7_MINI, 1., Mlim_Fstar_MINI, 0., user_params->FAST_FCOLL_TABLES);
                                        fcoll_MINI *= pow(10., 10.);
                                    }
                                    else
                                        fcoll = Nion_ConditionalM(zpp_growth[R_ct], log(M_MIN), log(Mmax), sigmaMmax, Deltac, curr_dens, astro_params->M_TURN, astro_params->ALPHA_STAR, 0., astro_params->F_STAR10, 1., Mlim_Fstar, 0., user_params->FAST_FCOLL_TABLES);
                    ...
                    #pragma omp parallel shared(dxheat_dt_box, dxion_source_dt_box, dxlya_dt_box, dstarlya_dt_box, dfcoll_dz_val, del_fcoll_Rct, freq_int_heat_tbl_diff,                                                                                \
                            ...
                            dstarlya_dt_prefactor_MINI, dstarlyLW_dt_prefactor_MINI, prefactor_2_MINI, const_zp_prefactor_MINI, exotic_energy_injected) \
                            private(box_ct, x_e, T, dxion_sink_dt, dxe_dzp, dadia_dzp, dspec_dzp, dcomp_dzp, dxheat_dzp, J_alpha_tot, dxheat_DM_dzp, dxlya_DM_dt, dxion_DM_dt, eps_DM, T_inv, T_inv_sq, \
                            xc_fast, xi_power, xa_tilde_fast_arg, TS_fast, TSold_fast, xa_tilde_fast, dxheat_dzp_MINI, J_alpha_tot_MINI, curr_delNL0)  num_threads(user_params->N_THREADS)
                    {
                    

                        // llh not sure
                        #pragma omp for reduction(+ : J_alpha_ave, xalpha_ave, Xheat_ave, Xion_ave, Ts_ave, Tk_ave, x_e_ave, J_alpha_ave_MINI, Xheat_ave_MINI, J_LW_ave, J_LW_ave_MINI, Lambda_ion_DM_ave, eps_DM_ave, J_alpha_DM_ave)
                        for (box_ct = 0; box_ct < HII_TOT_NUM_PIXELS; box_ct++)
                        {
                            ...
                            if (R_ct == 0)
                            {
                                ...
                                dxion_DM_dt = f_H / (NUIONIZATION * hplank) * exotic_energy_injected->f_H_ION * exotic_energy_injected->Inj_ENERGY_SMOOTH + f_He / (HeI_NUIONIZATION * hplank) * exotic_energy_injected->f_He_ION * exotic_energy_injected->Inj_ENERGY_SMOOTH; // llh DM ionis contrib  (Eq. 13 LLH+16)
                                dxe_dzp = dt_dzp * ( (flag_options->USE_MINI_HALOS) ? (dxion_source_dt_box[box_ct] + dxion_DM_dt + dxion_source_dt_box_MINI[box_ct] - dxion_sink_dt) :  (dxion_source_dt_box[box_ct] + dxion_DM_dt - dxion_sink_dt) ); // llh

                                // Next, let's get the temperature components
                                // Note that k_B is given in erg/K
                                dadia_dzp = 3 / (1.0 + zp);
                                dadia_dzp += (fabs(curr_delNL0) > FRACT_FLOAT_ERR) ? (dgrowth_factor_dzp / (1.0 / curr_delNL0 + growth_factor_zp)) : 0.; // add adiabatic heating/cooling from structure formation
                                dadia_dzp *= (2.0 / 3.0) * T; // Adiabatic term
                                dspec_dzp = - dxe_dzp * T / (1 + x_e); // Heating due to the changing species
                                dcomp_dzp = dcomp_dzp_prefactor * (x_e / (1.0 + x_e + f_He)) * (Trad_fast - T); // Compton heating: dcomp_dzp = dT_comp(zp, T, x_e);
                                dxheat_dzp = dxheat_dt_box[box_ct] * dt_dzp * 2.0 / 3.0 / k_B / (1.0 + x_e); // X-ray heating
                                eps_DM = (flag_options->USE_DM_ENERGY_INJECTION) ? (exotic_energy_injected->f_HEAT * exotic_energy_injected->Inj_ENERGY_SMOOTH) : 0.; // in egs/s
                                dxheat_DM_dzp = eps_DM * dt_dzp * 2.0 / 3.0 / k_B / (1.0 + x_e);  // llh eventually DM heating (Eq. 14 LLH+16) -> in K
                                dxheat_dzp_MINI = (flag_options->USE_MINI_HALOS) ? (dxheat_dt_box_MINI[box_ct] * dt_dzp * 2.0 / 3.0 / k_B / (1.0 + x_e)) : 0.;
                                ...
                                if (T < MAX_TK)
                                    T += dzp * ( (flag_options->USE_MINI_HALOS) ? (dxheat_dzp + dxheat_DM_dzp + dxheat_dzp_MINI + dcomp_dzp + dspec_dzp + dadia_dzp) : (dxheat_dzp + dxheat_DM_dzp + dcomp_dzp + dspec_dzp + dadia_dzp) );
                
                                if (T < 0)
                                    T = T_cmb * (1 + zp); // spurious bahaviour of the trapazoidalintegrator. generally overcooling in underdensities
                                
                                this_spin_temp->x_e_box[box_ct] = x_e;
                                this_spin_temp->Tk_box[box_ct] = T;

                                // llh ly_alpha_DM contrib prefactor_1 is the baryon nb density
                                dxlya_DM_dt = prefactor_1 * C / FOURPI / Ly_alpha_HZ / hubble(zp) / hplank / Ly_alpha_HZ * exotic_energy_injected->f_EXC * exotic_energy_injected->Inj_ENERGY_SMOOTH; // (Eq. 15 LLH+16) 

                                J_alpha_tot = (dxlya_dt_box[box_ct] + dstarlya_dt_box[box_ct] + dxlya_DM_dt);  // llh  not really d/dz, but the lya flux 
                                ...
                                xa_tilde_fast_arg = ( (flag_options->USE_MINI_HALOS) ? (J_alpha_tot + J_alpha_tot_MINI) : J_alpha_tot ) * xa_tilde_prefactor * pow(1.0 + 2.98394 * xi_power + 1.53583 * xi_power * xi_power + 3.85289 * xi_power * xi_power * xi_power, -1.) ;
                                
                                // Must use WF effect -> New in v1.4
            ...
            else // If (flag_options->USE_MASS_DEPENDENT_ZETA == False)
            {
                ...
                #pragma omp parallel shared(previous_spin_temp, x_int_XHII, inverse_diff, delNL0_rev, dens_grid_int_vals, ST_over_PS, zpp_growth, dfcoll_interp1,                                                                                                                                                                             \
                            ...
                            dt_dzp, dgrowth_factor_dzp, dcomp_dzp_prefactor, this_spin_temp, xc_inverse, TS_prefactor, xa_tilde_prefactor, Trad_fast_inv, exotic_energy_injected)  \
                            private(box_ct, x_e, T, xHII_call, m_xHII_low, inverse_val, dxheat_dt, dxion_source_dt, dxlya_dt, dxlya_DM_dt, eps_DM, dxion_DM_dt, dstarlya_dt, curr_delNL0, R_ct, \
                            dfcoll_dz_val, dxion_sink_dt, dxe_dzp, dadia_dzp, dspec_dzp, dcomp_dzp, J_alpha_tot, T_inv, T_inv_sq, xc_fast, xi_power,                                   \
                            xa_tilde_fast_arg, TS_fast, TSold_fast, xa_tilde_fast)  num_threads(user_params->N_THREADS)
                {
                    // llh not sure I can do this...
                    //#pragma omp for reduction(+:J_alpha_ave,xalpha_ave,Xheat_ave,Xion_ave,Ts_ave,Tk_ave,x_e_ave)
                    #pragma omp for reduction(+ : J_alpha_ave, xalpha_ave, Xheat_ave, Xion_ave, Ts_ave, Tk_ave, x_e_ave, Lambda_ion_DM_ave, eps_DM_ave, J_alpha_DM_ave)
                    for (box_ct = 0; box_ct < HII_TOT_NUM_PIXELS; box_ct++)
                    {
                        x_e = previous_spin_temp->x_e_box[box_ct];
                        T = previous_spin_temp->Tk_box[box_ct];
                        ...
                        // llh ly_alpha_DM contrib prefactor_1 is the baryon nb density
                        // C is in cm s^{-1}
                        // prefactor_1 is in cm^{-3}
                        // hubble(zp) in s^{-1}
                        // hplanck in erg s^{-1}
                        // Result here in cm^{-2} s^{-1} Hz^{-1} sr^{-1} 
                        dxlya_DM_dt = prefactor_1 * C / FOURPI / Ly_alpha_HZ / hubble(zp) / hplank / Ly_alpha_HZ * exotic_energy_injected->f_EXC * exotic_energy_injected->Inj_ENERGY_SMOOTH;

                        // Now we can solve the evolution equations  //

                        // First let's do dxe_dzp 
                        dxion_sink_dt = alpha_A(T) * global_params.CLUMPING_FACTOR * x_e * x_e * f_H * prefactor_1 * (1. + curr_delNL0 * growth_factor_zp);
                        dxion_DM_dt = f_H / (NUIONIZATION * hplank) * exotic_energy_injected->f_H_ION * exotic_energy_injected->Inj_ENERGY_SMOOTH + f_He / (HeI_NUIONIZATION * hplank) * exotic_energy_injected->f_He_ION * exotic_energy_injected->Inj_ENERGY_SMOOTH; // llh DM ionis contrib            
                        dxe_dzp = dt_dzp * (dxion_source_dt + dxion_DM_dt - dxion_sink_dt); // llh added DM
                        ...
                        dadia_dzp *= (2.0 / 3.0) * T; // Adiabatic term
                        dspec_dzp = -dxe_dzp * T / (1 + x_e); // Heating due to the changing species
                        dcomp_dzp = dcomp_dzp_prefactor * (x_e / (1.0 + x_e + f_He)) * (Trad_fast - T); // Compton heating
                        dxheat_dzp = dxheat_dt * dt_dzp * 2.0 / 3.0 / k_B / (1.0 + x_e); // X-ray heating
                        eps_DM = exotic_energy_injected->f_HEAT * exotic_energy_injected->Inj_ENERGY_SMOOTH;
                        dxheat_DM_dzp = eps_DM * dt_dzp * 2.0 / 3.0 / k_B / (1.0 + x_e);   // llh eventually DM heating
                        ...
                        if (T < MAX_TK)
                            T += (dxheat_dzp + dcomp_dzp + dspec_dzp + dadia_dzp + dxheat_DM_dzp) * dzp; // llh added DM heating
                        ...
                        // llh added DM contrib
                        // J_alpha_tot = (dxlya_dt + dstarlya_dt); // not really d/dz, but the lya flux
                        J_alpha_tot = (dxlya_dt + dstarlya_dt + dxlya_DM_dt); // not really d/dz, but the lya flux
                        ...
                        J_alpha_star_ave += dstarlya_dt; //sj
                        J_alpha_x_ave += dxlya_dt; //sj
                        Lambda_ion_ave += dxion_source_dt; //sj
                        eps_heat_ave += dxheat_dt; //sj
                        J_alpha_DM_ave += dxlya_DM_dt;
                        eps_DM_ave += eps_DM;
                        Lambda_ion_DM_ave += dxion_DM_dt;
            ...
            if (LOG_LEVEL >= DEBUG_LEVEL)
            {
                ...
                if (flag_options->USE_MINI_HALOS)
                {
                    ...
                    // llh added DM variables HERE
                    LOG_DEBUG("zp = %e Ts_ave = %e x_e_ave = %e Tk_ave = %e J_alpha_ave = %e(%e) xalpha_ave = %e Xheat_ave = %e(%e) Xion_ave = %e J_LW_ave = %e (%e) Xion_DM=%e epsDM=%e JalphaDM=%e", zp, Ts_ave, x_e_ave, Tk_ave, J_alpha_ave, J_alpha_ave_MINI, xalpha_ave, Xheat_ave, Xheat_ave_MINI, Xion_ave, J_LW_ave / 1e21, J_LW_ave_MINI / 1e21, Lambda_ion_DM_ave, eps_DM_ave, J_alpha_DM_ave);
                }
                else
                    LOG_DEBUG("zp = %e Ts_ave = %e x_e_ave = %e Tk_ave = %e J_alpha_ave = %e xalpha_ave = %e Xheat_ave = %e Xion_ave = %e Xion_DM=%e epsDM=%e JalphaDM=%e", zp, Ts_ave, x_e_ave, Tk_ave, J_alpha_ave, xalpha_ave, Xheat_ave, Xion_ave, Lambda_ion_DM_ave, eps_DM_ave, J_alpha_DM_ave);
        ...
        // Were we are back in the main function (no loop or if statement)
...
void free_TsCalcBoxes(struct UserParams *user_params, struct FlagOptions *flag_options)
{
    ...
    if (flag_options->USE_MASS_DEPENDENT_ZETA)
    {
        ...
        // llh
        //free(dxlya_DM_dt_box);
        //free(dxheat_DM_dt_box);
        //free(dxion_DM_dt_box);
