...
#
# Manage the input_output and the database of files
...
class DatabaseManager : 
    ...
    def __init__(self, path=None, cache_path=None):
        self.path = path 
    ...
    def parse_args(self, parser) :
        ...
        parser.add_argument("-swave", "--DM_process_swave", help="set the DM process to swave", action="store_true")
        parser.add_argument("-decay", "--DM_process_decay", help="set the DM process to decay", action="store_true")
        ...
        parser.add_argument("-r", "--remove", nargs='*', type=int, help="Remove the following entries in the database (integers)")
        parser.add_argument("-sh", "--show", help="show the database", action="store_true")
        ...
        parser.add_argument("-infile", "--input_file", help="Inpute file of the models to treat")
        ...
        if args.input_file is not None :
            if any([par != None for par in parse_arr]) or args.show == True or args.remove is not None: 
                print("Use the -infile option without any other combination")
            else:  
                return self.read_input_file(args.input_file), force_overwrite, nomp

        if args.remove is not None :
            if any([par != None for par in parse_arr]) or args.show == True : 
                print("Use the -r option without any other combination")
                exit(0)
            self.remove_entries_database(args.remove)
            exit(0)

        if args.show is not None and args.show == True :
            if any([par != None for par in parse_arr]) or args.remove != None : 
                print("Use the -sh option without any other combination")
                exit(0)
        ...
        if swave == True and decay == True :
            print("Error: cannot set the process to both swave and decay")
            exit()
        ...
        for i in range(0, len(all_input[0])) :
            ...
            if mod.is_valid():
                ...
                print("Errod: trying to run an invalid model")
                exit(0)
            
        return input, force_overwrite, nomp
    ...
    def read_database(self) : 
        ...
        filename = self.path + "/.database.txt"
    ...
    def search_models(self, reference_model: model, models_arr: list[model]):
        """
    ...
    def get_models_database(self, index: list[int] or int, models_arr: list[model]=None) : 

        """
        Return the dictionnary of the model at position index (int)
        """
    ...
    def add_models_database(self, new_models: list[model], force_overwrite: bool=False, models_arr: list[model]=None) : 
        ...
        with open(filename, 'a') as f:
            if not file_database_existed and len(models_arr) == 0:
                print("# Database for:", self.path, file = f)
                print("# index bkr process mDM [eV] primary boost fs_method sigmav lifetime comment", file = f)
        ...
        for i in range(0, len(new_models)) : 
            ...
            if output_indices[i] >= len(models_arr) :
                ...
                with open(filename, 'a') as f:
                    print(temp_model.index, "\t", 
                        temp_model.bkr, "\t",
                        temp_model.process, "\t",
                        str("{:.2e}".format(temp_model.mDM)), "\t", 
                        temp_model.primary, "\t",
                        temp_model.boost, "\t",
                        temp_model.fs_method, "\t",
                        str("{:.2e}".format(temp_model.sigmav)), "\t",
                        str("{:.2e}".format(temp_model.lifetime)), "\t",
                        temp_model.comment, "\t", file=f)
    ...
    def remove_entry_database(self, index, models_arr: list[model] = None) :
        ...
        for i in range(index, len(models_arr)) : 
            if i == index : 
                ...
                if os.path.exists(self.path + "/result_run_" + str(models_arr[i].index) + ".txt") : 
                    os.remove(self.path + "/result_run_" + str(models_arr[i].index) + ".txt")
                else : 
                    print("The file:", self.path + "/result_run_" + str(models_arr[i].index) + ".txt", "does not exist in the first place.")
                    print("The database has missed a deletion ... entry only removed from the database file.")
                
                if os.path.exists(self.path + "/BrightnessTemp_" + str(models_arr[i].index)) : 
                    shutil.rmtree(self.path + "/BrightnessTemp_" + str(models_arr[i].index))

                if os.path.exists(self.cache_path + "/_cache_" + str(models_arr[i].index)) : 
                    shutil.rmtree(self.cache_path + "/_cache_" + str(models_arr[i].index))

                models_arr.pop(i)

            else : 
                if os.path.exists(self.path + "/result_run_" + str(models_arr[i-1].index) + ".txt") : 
                    os.rename(self.path + "/result_run_" + str(models_arr[i-1].index) + ".txt", self.path + "/result_run_" + str(models_arr[i-1].index-1) + ".txt")
                if os.path.exists(self.path + "/BrightnessTemp_" + str(models_arr[i-1].index)) : 
                    os.rename(self.path + "/BrightnessTemp_" + str(models_arr[i-1].index), self.path + "/BrightnessTemp_" + str(models_arr[i-1].index-1))
                if os.path.exists(self.cache_path + "/_cache_" + str(models_arr[i-1].index)) : 
                    os.rename(self.cache_path + "/_cache_" + str(models_arr[i-1].index), self.cache_path + "/_cache_" + str(models_arr[i-1].index-1))
                models_arr[i-1].index = models_arr[i-1].index - 1
                


    def write_database(self, models_arr: list[model], path: str =None):

        # We rewrite the entire database file
        if path is None:
            filename = self.path + "/.database.txt"
        else:
        ...
        with open(filename, 'w') as f:
            ...
            for mod in models_arr :
                print(mod.index, "\t", 
                    mod.bkr, "\t", 
                    mod.process, "\t",
                    str("{:.2e}".format(mod.mDM)), "\t", 
                    mod.primary, "\t",
                    mod.boost, "\t",
                    mod.fs_method, "\t",
                    str("{:.2e}".format(mod.sigmav)), "\t",
                    str("{:.2e}".format(mod.lifetime)), "\t",
                    mod.comment, "\t", file=f)
    ...
    def remove_entries_database(self, indices, force_deletion = False) : 
        ...
        self.write_database(models_arr)
    ...
    def print_f_vs_rs(self, evolve_data, inj_energy_smooth, input) :
        ...
        filename = self.path + "/result_run_" + str(input['index']) + ".txt"
        
        with open(filename, 'w') as f:

            print("# bkr process mDM [eV] primary boost fs_method sigmav (cm3/s) lifetime (s) comment", file = f)
            print("#", 
                input.bkr, "\t",
                input.process, "\t",
                str("{:.2e}".format(input.mDM)), "\t", 
                input.primary, "\t",
                input.boost, "\t",
                input['fs_method'], "\t",
                str("{:.2e}".format(input.sigmav)), "\t",
                str("{:.2e}".format(input.lifetime)), "\t",
                input['comment'], "\t", file=f)
    ...
    def print_f_vs_rs_from21cmFAST(self, evolve_data, input) :
        ...
        with open(filename, 'w') as ff:
            ...
            print("# process mDM [eV] primary boost fs_method comment", file = ff)
            print("#", input.process, "\t",
                 str("{:.2e}".format(input.mDM)), "\t", 
                input.primary, "\t",
                input.boost, "\t",
                input.fs_method, "\t",
                input.comment, "\t", file=ff)
    ...
    def read_input_file(self, filename:str) -> list[model]: 
